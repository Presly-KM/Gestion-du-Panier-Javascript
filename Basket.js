class Basket {                                                // On crée une classe Basket qui va nous permettre de gérer le panier
    constructor() {                                           // On crée le constructeur de la classe Basket. On va ici récupérer le panier du local storage et l'initialiser dans la classe
        ...
        if (basket == null) {                                 // Si le panier est null (c'est à dire qu'il n'existe pas encore du au fait que quand l'utilisateur arrive sur le site,logiquement, il n'a pas encore eu à utiliser le panier, par exemple en ajoutant des produits à l'intérieur) on fait alors comprendre a la console que cela signifie que le panier est probablement vide.
            ...
            this.basket = JSON.parse(basket);                 // On va donc transformer le panier en un tableau d'objets (on va donc désérialiser le panier) pour pouvoir l'utiliser dans la classe. On utilise JSON.parse pour transformer le panier en un tableau d'objets. Le panier est donc un tableau d'objets (c'est à dire qu'il contient des produits)
        }
    }
    ...
    add(product) {                                          // On crée la fonction d'ajout au panier. Il permet d'ajouter un produit au panier. Il prend en paramètre le produit à ajouter. Permettra lorsqu'on appellera la focntion de dire : c'est ce produit là que je veux ajouter au panier.
        let foundProduct = this.basket.find(p => p.id = product.id);   // Est ce que le produit existe déjà dans le panier ? On cherche dans le panier s'il y'a un produit dont l'id est egal a l'id du produit qu'on veut ajouter dans le panier 
        ...
        this.save();                                       // Une fois qu'on a ajouté le produit dans le panier on enregistre le panier mis à jour dans le local storage
    }


    remove(product) {
        this.basket = this.basket.filter(p => p.id != product.id);          // On filtre le panier pour ne garder que les produits dont l'id est différent de l'id du produit qu'on veut supprimer (on va donc supprimer le produit du panier)
        this.save();                                              // On enregistre le panier mis à jour dans le local storage
    }


    changeQuantity(product, quantity) {
        let foundProduct = this.basket.find(p => p.id = product.id);   // On veut trouver le produit en question dans le panier. On cherche donc dans le panier s'il y'a un produit dont l'id est egal a l'id du produit qu'on veut modifier la quantité
        if (foundProduct != undefined) {                          // Si le produit est trouvé dans le panier, on va modifier sa quantité
            ...
            if (foundProduct.quantity <= 0) {                       // Si la quantité du produit est inférieure ou égale à 0 (c'est à dire que l'utilisateur a voulu supprimer le produit du panier) on va le supprimer du panier
                this.remove(foundProduct);                   // On appelle la fonction removeFromBasket pour supprimer le produit du panier
            } else {                                            // Sinon (c'est à dire si la quantité du produit est supérieure à 0) on va juste enregistrer le panier mis à jour dans le local storage
                this.save();                               // On enregistre le panier mis à jour dans le local storage
            }
        }
    }
    ...
    getNumberProduct() {                                          // On crée la fonction qui va nous permettre de récupérer le nombre de produits dans le panier
